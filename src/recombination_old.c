...
void compute_number_recombinations(grid_t *thisGrid, confObj_t simParam, char *filename, dens_table_t *thisDensTable, redshift_table_t *thisRedshiftTable)
{
	...
	norm_pdf = read_table_norm_pdf(filename);
	
	printf("created norm\n");
	
	dens_table = create_table_dens(thisDensTable);
	
	printf("done denstable\n");
	
	redshift_table = create_table_redshift(thisRedshiftTable, simParam);
	
	printf("tables created \n");
	
	for(int i=0; i<1/*local_n0*/; i++)
	{
		printf("i = %d\n",i);
		for(int j=0; j<1/*nbins*/; j++)
		{
			for(int k=0; k<1/*nbins*/; k++)
			{
				dens = creal(thisGrid->igm_density[i*nbins*nbins+j*nbins+k]);
				photHI = creal(thisGrid->photHI[i*nbins*nbins+j*nbins+k]);
				thisGrid->nrec[i*nbins*nbins+j*nbins+k] =  get_nrec_history(simParam, norm_pdf, thisDensTable, dens_table, thisRedshiftTable, redshift_table, dens, photHI, temp, zstart, redshift)+0.*I;
