...
for i in range(len(redshift)-1):
    ...
    if(minimum > np.min(powerspec_21[1:-1]*kmid_bins_21[1:-1]**3*k)):
        if(np.min(powerspec_21[1:-1]*kmid_bins_21[1:-1]**3*k) > 0):
            minimum = np.min(powerspec_21[1:-1]*kmid_bins_21[1:-1]**3*k)
    if(maximum < np.max(powerspec_21[1:-1]*kmid_bins_21[1:-1]**3*k)):
        if(np.max(powerspec_21[1:-1]*kmid_bins_21[1:-1]**3*k) < float('inf')):
            maximum = np.max(powerspec_21[1:-1]*kmid_bins_21[1:-1]**3*k)
